...
`test_reasoning_ladder`, `test_function_reuse`, `test_checkpoint`. All 26 pass as of this handover.
...
- **The `--verify` audit feature (`verify.py`, `prompts.py`, `run_core.py`; `tests/test_verify.py`).** A fresh independent second pass over a finished run: claim extraction, audit under a hard-coded stress battery, reconciliation into one corrected briefing (originals kept alongside). Built deliberately not on `--extend` so the audit inherits no evidence chain and cannot launder the original's artifacts. Section 2 and the README describe the live behavior; the reconciliation prompt carries hard-won deference rules (treat a refutation as decisive only when the audit reproduces the discrepancy at the original's level of analysis and states each side's computation, and never adopt an unreproduced mechanism as fact).
...
**Scope (v1).** Fresh compute runs only; `--resume`, `--extend`, and `--verify` are blocked with a clear message (each would need the mode persisted in the saved state). `tests/test_compute.py` and `tests/test_compute_cli.py` drive `run_investigation` and the full `run_core.main()` respectively.
...
- **Compute mode is fresh-runs-only and gate-free (6.9).** `--compute` disables the G1-G4
  ...
  `call_with_ladder` at the shared 64k, and the same `llm.call` clamp brings any Anthropic call
