...
# Catches runaway computations (large bootstrap, iterative model fits)
# before they exhaust memory and crash the process. Set generously —
# legitimate mixed-effects models on large datasets can take 2-3 minutes.
...
class CodeExecutor:
    ...
    def execute(self, code, df, analysis_dir=None):
        ...
        # Set execution timeout (Unix only — silently skipped on Windows)
        prev_handler = None
        try:
            ...
            prev_handler = None  # signal.SIGALRM not available (Windows)
